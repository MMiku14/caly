...
fn replay(log: &OperationLog, options: &CliOptions, target: &str) -> ExitCode {
    ...
    // Replay is identical to the operator re-typing the command: the full
    // parse (incl. alias expansion) and dispatch path run again, and the
    // replayed operation is recorded once more (shell-history behaviour).
    // `history` itself is not recordable, so this cannot recurse.
